...
//¨Promesse pour la saisie du contrat
function pContrat(){
  return new Promise((resolve,reject)=>{
    document.getElementById('OK').onclick=()=>{
      let contrat={
        'valeur':document.getElementById('valeurcontrat').value,
        'couleur':"",
        'coinche':document.getElementById('coinche').value
      };
      let radioCouleur=document.getElementsByName('couleurcontrat');
      for (let i=0;i<radioCouleur.length;i++){
        if (radioCouleur[i].checked){contrat.couleur=radioCouleur[i].value};
      }
      if (contrat.couleur!=""){resolve(contrat)};
    }
  })
}


var CarteOnclick=new Proxy(canvas,{
  on:
})
var cliqueCarte=new Promise((resolve,reject)=>{
  if (options.target && !(options.target.groupe) && options.target.top<=70*vh){
  ...
  } else {console.log('pas une carte');reject()}
})

canvas.on('mouse:up',cliqueCarte);

//Promesse de pose d'une carte
function pCarte(){
  return new Promise((resolve,reject)=>{
...
//Promesse de ramassage d'un pli
function pPli(){
  return new Promise((resolve,reject)=>{
    canvas.on('mouse:up',(options)=>{
      if (options.target && options.target.groupe){//on clique sur un pli
        resolve();
      }
    })
  })
...
socket.on('sallePleine',fn=>{
  affichePage("loginpage");
  fn();
})

socket.on('refresh',no=>{
  nojoueur=no;
  affichePage("mainpage");
})

socket.on('add user',fn=>{
  affichePage('loginpage');
  const promiseConnect=new Promise((resolve,reject)=>{
    for (let i=0;i<4;i++){
      document.getElementById('Entrer'+i).onclick=()=>{
        let nomjoueur=document.getElementById('joueur'+i).value;
        if (nomjoueur!=""){
          nojoueur=i;
          resolve([nomjoueur,i])};
      }
  ...
  promiseConnect.then(([nomjoueur,i])=>{
    fn([nomjoueur,i]);
    affichePage("mainpage");
  });
})

socket.on('new user',data=>{MAJJoueurs(data)});

socket.on('phase',(phase,fn)=>{
  switch (phase){
    case 'annonces':
      for (let i in imagesCartes){
      ...
      afficheMain();
      pBouton('Jeprends').then((data)=>fn(data));
      pBouton('Redonner').then((data)=>fn(data));
    break;
    ...
    case 'saisieContrat':
      ...
      pContrat().then((data)=>fn(data));
    break;

    case 'montrerJeux':
      affichePage("mainpage","nouvelledonne");
      afficheMainsPosees();
      pBouton('nlledonne').then(()=>{
        affichePage("mainpage");
        fn()});
    break;    
    ...
    case 'afficheScores':
      ...
      pBouton('donneSuivante').then(()=>{
        let scores=[document.getElementById("score0").value,
...
function getData(){
  return new Promise((resolve,reject)=>{
  socket.emit('getData',data=>resolve(data));
})
}
...
function getMains(){
  return new Promise((resolve,reject)=>{
  socket.emit('getMains',data=>resolve(data));
})
}
...
//ramassage du pli par le joueur no
function plifait(){
  getData().then(data=>{
    let pli=data.pli;
    let no=data.ramasseur;
    let compteur=0;
    for (let i=0;i<4;i++){
...
function MAJJoueurs(joueurs){
  ...
  for (let i=0;i<4;i++){
    if (joueurs[i]) {
      ...
      document.getElementById('joueur'+i).value=joueurs[i];
    } else {
  ...
  jGauche.text = (joueurs[(nojoueur+1)%4]==null ? "" : joueurs[(nojoueur+1)%4]);
  jFace.text = (joueurs[(nojoueur+2)%4]==null ? "" : joueurs[(nojoueur+2)%4]);
  jDroite.text = (joueurs[(nojoueur+3)%4]==null ? "" : joueurs[(nojoueur+3)%4]);
  canvas.renderAll();
}
